...
const skinIdToNumber = (id, championKey) => {
  ...
  const numberAsString = idString.replace(championKey.toString(), '').replace(/0?/, '')
...
const createShortName = (skinName, championName) => {
  const proposedShortName = skinName.replace(championName, '').replace('  ', ' ').trim()
...
const start = async () => {
  ...
  const championResponse = await fetchJson(`https://ddragon.leagueoflegends.com/cdn/${currentVersion}/data/en_US/champion.json`)
  const champions = []
  ...
  // const championsToUSe = [champions[0], champions[1], champions[2]]

  const championsWithSkins = await Promise.all(champions.map(champion => new Promise(async (resolve, reject) => {
    const moreChampionData = await fetchJson(`https://cdn.communitydragon.org/${currentVersion}/champion/${champion.key}/data`)
  ...
  console.log(JSON.stringify(championsWithSkins, null, 2))
}
